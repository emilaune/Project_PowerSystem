...
# updated 08.11.21 13:00
    # calculation of power transmissions: |Vi||Vj|e^(di+(-dj))
    # added operating capacitance to transmitted power (line 104)
    # added np.abs() to Q_loss (line 110)
    # corrected incorrect use of z_ij to (z_ij)^* (line 106)
    # added np.abs() to P_loss (line 110)
    # transmitted power w/OC - added conjugate to impedance...
...
def powerTransmissions(linedata,busdata,YBus):
    ...
    for i in range(0,len(linedata)):
        ...
        [yOCMagn,yOCAng] = cmath.polar(complex(0,linedata[i][4]/2))
        S_ijOC = ((busdata[linedata[i][0]-1][4])**2)*complex(yOCMagn*np.cos(yOCAng),-yOCMagn*np.sin(yOCAng))
        ...
        [dMagnitude,dAngle] = cmath.polar(YBus[linedata[i][0]-1][linedata[i][1]-1]) #-1*YBus[linedata[i][0]-1][linedata[i][1]-1]
        dLoss = complex(dMagnitude*np.cos(dAngle),-1*dMagnitude*np.sin(dAngle))
        S_ij_loss = numerator*dLoss*Sbase
