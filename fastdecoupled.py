...
def powerTransmissions(linedata,busdata,YBus):
    ...
    for i in range(0,len(linedata)):
        ...
        # transmitted power
        angles = complex(np.cos(busdata[linedata[i][0]-1][5]-(-busdata[linedata[i][1]-1][5])),np.sin(busdata[linedata[i][0]-1][5]-(-busdata[linedata[i][1]-1][5])))     #cos(d_i-d_j) + j*sin(d_i-d_j)
        V = (busdata[linedata[i][0]-1][4]**2)-busdata[linedata[i][0]-1][4]*busdata[linedata[i][1]-1][4]*angles  #|V_i|**2-|V_i|*|V_j|*(cos(d_i-(-d_j)) + j*sin(d_i-(-d_j))
        Y = (-1)*A*(complex(np.cos(angle),-np.sin(angle)))
        S_ij = V*Y*Sbase
        ...
        denominator = -1*YBus[linedata[i][0]-1][linedata[i][1]-1]
        S_ij_loss = numerator*denominator*Sbase
...
def decoupledJacobian(YBus,busdata,nGen,nLoad):
    dim = len(busdata)
    J1 = np.zeros([nGen + nLoad, nGen + nLoad])
    J2 = np.zeros([nGen + nLoad, nLoad])
    J3 = np.zeros([nLoad, nGen + nLoad])
            ...
            for j in range(0, dim):
                if (busdata[j][1] == 1) or (busdata[j][1] == 2):  # find load or generator bus
                    ...
                    if (i != j) and (A == 0):
                        ...
                        J1[j1count[0], j1count[1]] = busdata[i][4] * busdata[j][4] * A * np.sin(-angle)
                        j1count[1] += 1
                    elif (i == j):
                        productP1 = 0
                        for k in range(0, nGen + nLoad + 1):
                            [A, angle] = cmath.polar(YBus[i][k])
                            if (i != k) and (A != 0):
                                productP1 = productP1 + A * busdata[k][4] * np.sin(-angle)
                        J1[j1count[0], j1count[1]] = -1 * busdata[i][4] * productP1
                        j1count[1] += 1
            j1count[0] += 1
            ...
            for j in range(0, dim):
                if (busdata[j][1] == 2):
                    ...
                    if (i != j) and (A == 0):
                        ...
                        J2[j2count[0], j2count[1]] = busdata[i][4] * A * np.cos(0)
                        j2count[1] += 1
                    elif (i == j):
                        productP2 = 0
                        for k in range(0, nGen + nLoad + 1):
                            [Ak, anglek] = cmath.polar(YBus[i][k])
                            if (Ak != 0):
                                productP2 = productP2 + Ak * busdata[k][4] * np.cos(
                                    busdata[i][5] - busdata[k][5] - anglek)
                        J2[j2count[0], j2count[1]] = busdata[i][4] * Ai * np.cos(anglei) + productP2
                        j2count[1] += 1
            j2count[0] += 1
            ...
            for j in range(0, dim):
                if (busdata[j][1] == 1) or (busdata[j][1] == 2):
                    ...
                    if (i != j) and (A == 0):
                        ...
                        J3[j3count[0], j3count[1]] = -busdata[i][4] * A * np.cos(-angle)
                        j3count[1] += 1
                    elif (i == j):
                        productQ1 = 0
                        for k in range(0, nGen + nLoad + 1):
                            [Ak, anglek] = cmath.polar(YBus[i][k])
                            if (i != k) and (Ak != 0):
                                productQ1 += productQ1 + Ak * busdata[i][4] * np.cos(-anglek)
                        J3[j3count[0], j3count[1]] = busdata[i][4] * productQ1
                        j3count[1] += 1
            j3count[0] += 1
                ...
                if (busdata[j][1] == 2):
                    ...
                    if (i != j) and (A == 0):
                        ...
                        for k in range(0, nGen + nLoad + 1):
                            [Ak, anglek] = cmath.polar(YBus[i][k])
                            if (Ak != 0):
                                productQ2 = productQ2 + Ak * busdata[k][4] * np.sin(
                                    busdata[i][5] - busdata[k][5] - anglek)
                        J4[j4count[0], j4count[1]] = -busdata[i][4] * np.sqrt(
                            YBus[i][i].real ** 2 + YBus[i][i].imag ** 2) * np.sin(
                            np.arctan(YBus[i][i].imag / YBus[i][i].real)) + productQ2
                        j4count[1] += 1
            j4count[0] += 1
            J2 = np.zeros([nGen + nLoad, nLoad])
            J3 = np.zeros([nLoad, nGen + nLoad])
...
def fastdecoupled():
    ...
    while (b == False):  # while not converged
        ...
        # Find new Jacobian
        J = decoupledJacobian(YBus, busdata, nGen, nLoad)
        printJacobian(J)
        inverse = np.linalg.inv(J)

        # Update the state variables
        [myangles, myVs, busdata] = updateStateVariables(inverse, myangles, myVs, pMismatch, qMismatch, busdata, nGen,
                                                         nLoad)
